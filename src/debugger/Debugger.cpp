...
Debugger::Debugger(std::shared_ptr<CPU> cpu, std::shared_ptr<MMU> mmu) : m_cpu(cpu), m_mmu(mmu) {}
...
void Debugger::run(std::ostream &output_stream) {
    ...
    while (true) {
        if (step_mode) {
            output_stream << GeneralUtilities::formatted_string("[PC: 0x%04X]> ", m_cpu->get_16_bit_register(CPU::Register::PC));
            ...
            case gbcemu::DebuggerCommand::Command::ClearBreakpoint:
                output_stream << "clear breakpoint" << std::endl;
                continue;
