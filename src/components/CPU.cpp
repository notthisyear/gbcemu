...
CPU::CPU(std::shared_ptr<MMU> const mmu, std::shared_ptr<PPU> const ppu, bool const output_trace) : m_mmu(mmu), m_ppu(ppu), m_output_trace(output_trace) {
    ...
    if (m_output_trace) {
        char *const full_path = new char[CPU::kMaxPathLength];
        std::size_t const size{ GetModuleFileNameA(NULL, full_path, CPU::kMaxPathLength) };
        if (size < CPU::kMaxPathLength) {
            std::string const s{ GeneralUtilities::formatted_string("%s\\%s", GeneralUtilities::get_folder_name_from_full_path(full_path),
                                                                    CPU::kTraceFileName) };
            m_trace_stream = std::ofstream(s);
        } else {
            LogUtilities::log_error(std::cout,
                                    GeneralUtilities::formatted_string("Path to executable cannot be longer than %d characters", CPU::kMaxPathLength));
            std::exit(1);
        }
    }
}
...
void CPU::tick() {
    ...
    if (m_current_cpu_phase_tick_count == 0) {
        ...
        if (run_state_machine) {
            ...
            case CPU::State::InterruptPushPC:
                m_current_interrupt_phase_counter++;
                if (m_current_interrupt_phase_counter == 2) {
                    uint16_t const sp = get_16_bit_register(CPU::Register::SP);
                    (void)m_mmu->try_map_data_to_memory((uint8_t *)&m_reg_pc, sp - 2U, 2U);
    ...
    m_current_cpu_phase_tick_count = (m_current_cpu_phase_tick_count + 1) & (kExecutionTicksPerOperationStep - 1);
    m_tick_ctr++;
}
...
bool CPU::should_handle_interrupt() {
    uint8_t const interrupt_enable{ m_mmu->get_io_register(MMU::IORegister::IE) };
    uint8_t interrupt_flag{ m_mmu->get_io_register(MMU::IORegister::IF) };

    bool const IME_was_set{ m_interrupt_enabled };

    // A pending interrupt can only be serviced if IME is set and the corresponding bit in the IE register is set
    bool const pending_interrupt_can_be_serviced{ IME_was_set && ((interrupt_enable & interrupt_flag) > 0x00) };
    if (pending_interrupt_can_be_serviced) {
        m_interrupt_enabled = false;

        // Let's find which interrupt that was non-zero
        std::size_t const last_interrupt_idx{ static_cast<std::size_t>(CPU::InterruptSource::Joypad) };
        for (std::size_t i{ 0 }; i <= last_interrupt_idx; ++i) {
    ...
    return pending_interrupt_can_be_serviced;
}

void CPU::fetch_and_decode() {
    uint8_t const current_byte{ read_at_pc() };
    ...
    if (m_state == State::FetchAndDecode) {
        m_is_extended_opcode = OpcodeBuilder::is_extended_opcode(current_byte);
        if (!m_is_extended_opcode) {
            m_current_opcode = OpcodeBuilder::decode_opcode(current_byte, false);
        }
    } else if (m_state == State::FetchAndDecodeExtended) {
        m_current_opcode = OpcodeBuilder::decode_opcode(current_byte, true);
    }
}
...
void CPU::read_at_pc_and_store_in_intermediate(const CPU::Register reg) {
...
std::string CPU::disassemble_instruction_at(uint16_t current_pc, uint8_t &instruction_length) const {
    ...
    uint16_t const pc_at_start{ current_pc };
    m_mmu->try_read_from_memory(&current_instruction, current_pc, 1);
    current_pc++;

    auto is_extended = OpcodeBuilder::is_extended_opcode(current_instruction);
    if (is_extended) {
        m_mmu->try_read_from_memory(&current_instruction, current_pc, 1);
        current_pc++;
    }

    std::shared_ptr<Opcode> opcode = OpcodeBuilder::decode_opcode(current_instruction, is_extended);
...
void CPU::print_disassembled_instructions(std::ostream &stream, uint16_t const number_of_instructions) {
    uint16_t current_pc{ m_reg_pc };
    if (m_state == CPU::State::FetchAndDecode && m_next_instruction_preloaded)
        current_pc--;

    uint8_t last_instruction_length{ 0U };
    for (std::size_t i{ 0U }; i < number_of_instructions; ++i) {
...
void CPU::set_interrupt_enable(bool const on_or_off) {
    if (on_or_off)
        m_cycles_until_interrupts_enabled = kExecutionTicksPerOperationStep + 1;
    else
        m_interrupt_enabled = false;
}
...
void CPU::enable_breakpoint_at(uint16_t const pc) {
...
bool CPU::half_carry_occurs_on_subtract(uint8_t const v, uint8_t const value_to_subtract) const { return ((v & 0x0F) - (value_to_subtract & 0x0F)) & 0x10; }

bool CPU::half_carry_occurs_on_subtract_with_carry(uint8_t const v, uint8_t const value_to_subtract) const {
    return ((v & 0x0F) - (value_to_subtract & 0x0F) - (flag_is_set(CPU::Flag::C) ? 1 : 0)) & 0x10;
}

bool CPU::half_carry_occurs_on_add(uint8_t const v, uint8_t const value_to_add, bool const include_carry) const {
    return ((v & 0x0F) + (value_to_add & 0x0F) + (flag_is_set(CPU::Flag::C) & include_carry)) > 0x0F;
}

bool CPU::half_carry_occurs_on_add(uint16_t const v, uint16_t const value_to_add, bool const include_carry) const {
    return ((v & 0x0FFF) + (value_to_add & 0x0FFF) + (flag_is_set(CPU::Flag::C) & include_carry)) > 0x0FFF;
}

bool CPU::carry_occurs_on_add(uint8_t const v, uint8_t const value_to_add, bool const include_carry) const {
    return ((uint16_t)v + (uint16_t)(value_to_add) + (flag_is_set(CPU::Flag::C) & include_carry)) > 0xFF;
}

bool CPU::carry_occurs_on_add(uint16_t const v, uint16_t const value_to_add, bool const include_carry) const {
    return ((uint32_t)v + (uint32_t)value_to_add + (flag_is_set(CPU::Flag::C) & include_carry)) > 0xFFFF;
}

bool CPU::carry_occurs_on_subtract(uint16_t const v, uint16_t const value_to_subtract) const { return value_to_subtract > v; };
...
void CPU::print_flag_value(std::ostream &stream, std::string const &flag_name, bool const value, bool const insert_newline) const {
...
void CPU::print_reg(std::ostream &stream, const CPU::Register reg, bool const insert_newline) const {
