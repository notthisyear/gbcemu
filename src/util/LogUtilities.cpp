...
const std::unordered_map<LoggerType, std::string> LogUtilities::m_logger_names = {
    { LoggerType::Internal, "internal" },
    { LoggerType::Cpu, "cpu" },
    { LoggerType::Mmu, "mmu" },
    { LoggerType::Cartridge, "cartridge" },
};

std::unordered_map<LoggerType, std::shared_ptr<spdlog::logger>> &LogUtilities::loggers() {
...
void LogUtilities::log(LoggerType type, LogLevel log_level, const std::string &log_message) {
    auto logger = loggers().find(type);
    if (logger == loggers().end()) {
        loggers().emplace(type, spdlog::stdout_color_mt(m_logger_names.find(type)->second));
